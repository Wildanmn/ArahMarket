...
export const MarketHistoryView: React.FC<MarketHistoryViewProps> = ({ onOpenChart }) => {
  ...
  const [activeFilter, setActiveFilter] = useState<'ALL' | 'METALS_CRYPTO' | 'INDICES' | 'FOREX'>('ALL');
  ...
  const getAssetCategory = (sym: string): 'METALS_CRYPTO' | 'INDICES' | 'FOREX' => {
  ...
  const filteredAssets = useMemo(() => {
    ...
    return target13Keys.filter(key => {
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div className="p-3 bg-slate-900/60 border border-slate-800 rounded-lg">
              ...
              <div className="space-y-1.5 text-xs font-mono">
                {target13Keys.slice(0, 6).map(sym => {
                ...
                {target13Keys.slice(0, 6).map(sym => {
                  ...
                  return (
                    <div key={sym} className="flex items-center justify-between py-1 border-b border-slate-800/60">
                      ...
                      <span className="text-slate-300">${b.price.toLocaleString()}</span>
                    </div>
                  );
                })}
              </div>
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between pb-3 border-b border-slate-800/80 mb-4 gap-3">
            <div>
              ...
              <p className="text-xs text-slate-400 font-mono mt-1">
                Archived dossier for all 13 core markets &bull; Saved permanently in relational database
              </p>
            </div>
                    ...
                    <div className="flex items-baseline justify-between py-2 border-y border-slate-800/60 mb-2 font-mono">
                      <div>
                        ...
                        <span className="text-base font-bold text-slate-100 tabular-nums">
                          ${item.price > 100 ? item.price.toLocaleString() : item.price.toFixed(4)}
                        </span>
                      </div>
